...
function isPalindrome(str) {
  const news = str.toLowerCase().replace(/[\s.,%]/g, '');
  let newStr = '';
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].length > prev.length) maxL = arr[i];
    if (arr[i].length === prev.length) maxL = prev;
    prev = arr[i];
  }
  return maxL;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < newStr.length; i += 1) {
    if (newStr[i] === newStr[i].toUpperCase()) {
      ali += newStr[i].toLowerCase();
    } else {
      ali += newStr[i].toUpperCase();
    }
  }
  return ali;
}
